...
class MerchDetail(Base):
    ...
    edition = Column(String, nullable=True)  # free text, e.g. "Ver. 3"
    color_id = Column(UUID(as_uuid=True), ForeignKey("idol_colors.id", onupdate="CASCADE", ondelete="SET NULL"), nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)

    __table_args__ = (
        # Exactly one of idol_id/group_id (album_details allows either or both).
        CheckConstraint(
