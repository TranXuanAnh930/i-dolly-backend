...
# Company-scoped via the parent concert (ticket_type_service).
router = APIRouter(prefix="/ticket_types", tags=["Ticket Types"])

@router.post("/add", response_model=TicketTypeRead)
def add_new_ticket_type(data: TicketTypeCreate, current_user: Users = Depends(require_manager_or_admin), _: None = Depends(rate_limit(20, 60, user_key)), db: Session = Depends(get_db)) -> TicketType:
    try:
...
def get_ticket_type_by_id(id: uuid.UUID, db: Session = Depends(get_db)) -> TicketType:
...
def update_existing_ticket_type(id: uuid.UUID, data: TicketTypeUpdate, current_user: Users = Depends(require_manager_or_admin), _: None = Depends(rate_limit(20, 60, user_key)), db: Session = Depends(get_db)) -> TicketType:
    try:
...
def delete_existing_ticket_type(id: uuid.UUID, current_user: Users = Depends(require_manager_or_admin), _: None = Depends(rate_limit(20, 60, user_key)), db: Session = Depends(get_db)) -> MessageResponse:
    try:
