...
# Album products are created via /products/add_with_detail.
@router.post("/add", response_model=AlbumDetailRead)
def add_new_album_detail(data: AlbumDetailCreate, current_user: Users = Depends(require_manager_or_admin), _: None = Depends(rate_limit(20, 60, user_key)), db: Session = Depends(get_db)) -> AlbumDetail:
    try:
...
def get_album_detail_by_id(product_id: uuid.UUID, db: Session = Depends(get_db)) -> AlbumDetail:
...
def update_existing_album_detail(product_id: uuid.UUID, data: AlbumDetailUpdate, current_user: Users = Depends(require_manager_or_admin), _: None = Depends(rate_limit(20, 60, user_key)), db: Session = Depends(get_db)) -> AlbumDetail:
    try:
...
def delete_existing_album_detail(product_id: uuid.UUID, current_user: Users = Depends(require_manager_or_admin), _: None = Depends(rate_limit(20, 60, user_key)), db: Session = Depends(get_db)) -> MessageResponse:
    try:
