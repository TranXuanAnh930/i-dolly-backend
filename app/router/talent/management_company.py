...
from app.schema.talent.management_company import ManagementCompanyBase, ManagementCompanyCreate, ManagementCompanyRead
from app.services.talent.management_company_service import (
    add_company,
    delete_company,
    get_companies,
    get_company,
...
async def get_company_by_id(id: uuid.UUID, db: Session = Depends(get_db)):
...
async def update_existing_company(id: uuid.UUID, data: ManagementCompanyBase, current_user: Users = Depends(require_admin), db: Session = Depends(get_db)):
...
async def delete_existing_company(id: uuid.UUID, current_user: Users = Depends(require_admin), db: Session = Depends(get_db)):
